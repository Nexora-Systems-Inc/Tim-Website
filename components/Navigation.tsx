...
export default function Navigation() {
  ...
  const links = [n.collections, n.artists, n.exhibitions, n.about, n.contact];
    ...
    <>
      <motion.nav
        ...
        style={{
          ...
          boxShadow: scrolled ? "0 1px 0 rgba(184,150,90,0.12)" : "none",
        }}
      >
        <div className="container flex items-center justify-between h-[72px]">
          ...
          <a href="#" className="flex flex-col leading-none select-none">
            ...
            <span
              ...
              style={{ color: scrolled ? "var(--gold)" : goldAlpha(0.85) }}
            >
              {n.gallery_subtitle}
            </span>
          </a>
          ...
          <ul className="hidden md:flex items-center gap-9">
            {links.map((link) => (
              <li key={link}>
                <a
                  href="#"
                  className="relative text-[10.5px] tracking-[0.24em] uppercase transition-colors duration-350 group"
                  style={{ color: scrolled ? "var(--charcoal)" : ivoryAlpha(0.8) }}
                >
                  {link}
                  <span
                    className="absolute -bottom-px left-0 h-px w-0 transition-all duration-500 group-hover:w-full"
          ...
          <div className="hidden md:flex items-center gap-5">
            {/*
              FR/EN TOGGLE — add here when ready.
              Shape: <LocaleToggle /> — reads locale from I18nContext,
              calls a setLocale function passed down from layout.
            */}
            <a
              href="#"
              className="text-[10px] tracking-[0.28em] uppercase px-5 py-2 transition-all duration-450 border"
              style={{
                borderColor: scrolled ? goldAlpha(0.55) : goldAlpha(0.45),
                color: scrolled ? "var(--gold)" : goldAlpha(0.88),
              }}
              onMouseEnter={(e) => {
              ...
              onMouseLeave={(e) => {
                ...
                (e.currentTarget as HTMLElement).style.color = scrolled ? "var(--gold)" : goldAlpha(0.88);
                (e.currentTarget as HTMLElement).style.borderColor = scrolled ? goldAlpha(0.55) : goldAlpha(0.45);
              }}
            >
              {n.cta}
            </a>
